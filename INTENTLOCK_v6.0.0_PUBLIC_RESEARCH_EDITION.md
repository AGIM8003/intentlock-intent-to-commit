...
**Edition:** v6.0.0 Public Research Edition (uplift; v5.0 text preserved below and in companion file)  
**Publication date:** July 13, 2026  
**ORCID:** 0009-0002-3234-7765  
**DOI:** 10.5281/zenodo.21361708  
...
# v6.0.0 EVIDENCE UPLIFT (2026-07-17)

**Edition:** v6.0.0 Public Research Edition  
**ORCID:** 0009-0002-3234-7765  
**DOI (concept):** 10.5281/zenodo.21361708  
**GitHub:** https://github.com/AGIM8003/intentlock-intent-to-commit  
**Sole SSOT:** `INTENTLOCK_v6.0.0_PUBLIC_RESEARCH_EDITION.md`  
**Implementation status:** Importable research library + evidence suite (stdlib-only); not production, not peer reviewed, not independently replicated.

## 1. Honest Status Boundary

IntentLock is **not** a universal AI-agent security product, **not** a replacement for code review or SAST/DAST, **not** a claim that sandboxes alone stop prompt injection, **not** a certified supply-chain compliance regime, and **not** a production merge gate. It is a local-first **intent-to-commit** control and evidence architecture: a signed human Intent Contract compiles into a Diff Envelope and capability policy; the agent works in a non-authoritative workspace; acceptance occurs only at an independent boundary; a content-addressed proof capsule binds contract, policy, operations, and verdict for offline verification.

## 2. Three-Layer Novelty Declaration

### Layer 1 — Component Novelty

| Component | Novel? | Evidence |
|-----------|--------|----------|
| Human task authorization | PARTIAL | Common in review/PR workflows |
| Sandbox / permissioned agent | PARTIAL | Widely shipped |
| Diff size / path allowlists | PARTIAL | Policy-as-code, CODEOWNERS |
| Signed Intent Contract as acceptance root | YES | Contract digest binds task+paths+limits |
| Agent forbidden as self-acceptance authority | YES | Explicit UNAUTHORIZED on self-approve |
| Offline intent-to-commit proof capsule | YES | Capsule digest over contract+policy+units+verdict |

### Layer 2 — Integration Novelty

Existing stacks offer sandboxes **or** PR review **or** attestations. They miss the linked chain: **signed Intent Contract → compiled Diff Envelope → non-authoritative execution units → frozen candidate → independent acceptance → proof capsule**, with fail-closed verdicts (`OUT_OF_ENVELOPE`, `UNAUTHORIZED`, `CAPABILITY_EXCEEDED`).

### Layer 3 — Architectural Novelty

**Principle:** The producer of a change is never the authority that admits it; admission is envelope-bound and capsule-evidenced.  
**Examiner sentence:** IntentLock's distinguishing claim is the enforceable separation of agent production from independent, contract-bound acceptance with an offline-verifiable capsule — not sandboxing alone.

## 3. Negative Claim Register

1. Not a claim of complete prompt-injection immunity.
2. Not a replacement for human code review judgment.
3. Not a patent grant or freedom-to-operate opinion.
4. Not peer reviewed.
5. Not independently replicated by third parties.
6. Not a production merge-queue product.
7. Not a cryptographic HSM or hardware root of trust.
8. Not universal coverage of all agent tool APIs.
9. Not a substitute for SLSA/in-toto compliance certification.
10. Not an endorsement of any commercial coding agent.
11. Not a guarantee that Diff Envelopes capture semantic intent.
12. Not production performance SLAs (PoC timings only).

## 4. Competitive Positioning

| Approach | Strength | Defeat / gap IntentLock addresses |
|----------|----------|-----------------------------------|
| IDE agent sandboxes | Contain FS/network | Do not bind a specific signed task to the final commit |
| PR review alone | Human judgment | Reviewers see diffs after authority already blurred; no capsule |
| CODEOWNERS | Path ownership | Not task-scoped Intent Contracts or capability vectors |
| SLSA / in-toto | Build provenance | Build attestations ≠ human intent-to-commit binding |
| Policy-as-code (OPA) | General policy | Rarely encodes agent-non-authority + Diff Envelope + capsule |
| Signed commits (GPG) | Author identity | Signs result, not the approved envelope of the agent session |

**Response strategy:** Position IntentLock as the **acceptance boundary** between agent workspaces and protected refs, producing capsules auditors can verify offline.

## 5. Prior Art (12)

1. GitHub CODEOWNERS — path review gates.  
2. Gerrit / Phabricator mandatory review.  
3. Open Policy Agent — general policy.  
4. SLSA / in-toto — supply-chain attestations.  
5. Sigstore / cosign — signature tooling.  
6. Cursor/Copilot sandbox & hooks — agent containment.  
7. gVisor / Bubblewrap — OS sandboxes.  
8. SELinux / AppArmor — MAC.  
9. Conventional commits + protected branches.  
10. MCP security discussions — tool permissioning.  
11. Intent-based access control (IBAC) literature — related vocabulary, different artifact.  
12. Author's ALETHEIA / HRC-OT — complementary evidence/outcome layers, not intent-to-commit.

**Gap:** none combine signed task contracts, Diff Envelope limits, mandatory non-authority of the agent, and offline proof capsules as one deterministic loop.

## 6. Proof Sketches

**A — Totality of verdicts.** `classify_decision` returns exactly one of six verdicts for any finite candidate; branch order is exhaustive (auth → freeze → envelope → caps → empty → accept). Gate 8/8.

**B — Agent non-authority invariant.** If `agent_self_approved` is true, verdict is `UNAUTHORIZED` regardless of envelope fitness. Gate test `agent_self_approve_blocked`.

**C — Envelope soundness (paths).** If any touched path fails `path_allowed`, verdict is `OUT_OF_ENVELOPE` and cannot be `ACCEPTED`. Alt-impl oracle agrees.

## 7. Real-World Scenario (numbers)

Human authorizes: *fix typo in `docs/README.md`* (`max_files=1`, `max_ops=3`, capability `edit` only). Agent also edits `.github/workflows/ci.yml` and attempts `network`. IntentLock → `OUT_OF_ENVELOPE`. Corrected single-file candidate → `ACCEPTED`. Self-approved refactor → `UNAUTHORIZED`. Reproduced by `poc/intentlock_realworld.py`.

## 8. Standards Alignment (honest PARTIAL)

| Standard | Alignment | Note |
|----------|-----------|------|
| SLSA | PARTIAL | Capsules resemble attestations; not a SLSA level claim |
| in-toto | PARTIAL | Layout analogy; not a conforming implementation |
| EU AI Act Art. 13 transparency | PARTIAL | Machine-readable reasons; not a full dossier |
| ISO/IEC 27001 | NONE claimed | Deploy manifest lists requirements only |
| NIST SSDF | PARTIAL | Supports controlled change; not full SSDF adoption |
| OpenSSF Scorecard practices | PARTIAL | Protected-ref spirit; not a scorecard run |

## 9. Deployment Notes

See `poc/intentlock_deploy_manifest.json`. Stdlib library importable today. Security list items are **requirements, not implemented controls**.

## 10. Gap Register

| # | Gap | Severity |
|---|-----|----------|
| G1 | No third-party replication | High |
| G2 | No peer review | Medium |
| G3 | Semantic intent ≠ path allowlist | High |
| G4 | No IDE/agent product integration | Medium |
| G5 | No HSM-backed signatures in PoC | Medium |
| G6 | Capsule format not standardized | Low |
| G7 | **PDF REGENERATION NEEDED** | Low |

## 11. Version Note — v6.0.0

- Importable `poc/intentlock/` (`IntentLockEngine`).
- Evidence suite: PoC, Reality Gate 8/8, benchmark, alt-impl, mutation ≥90%, stress 2000 units, real-world, integration, quickstart.
- Header/DOI/ORCID updated; **NO SHRINK** — v5.0 content preserved above.

```text
INTENTLOCK_PUBLIC_RESEARCH_EDITION_v6_0_0
IMPORTABLE_REFERENCE_LIBRARY_ADDED
REALITY_GATE_8_OF_8
AGENT_NON_AUTHORITY_ENFORCED
